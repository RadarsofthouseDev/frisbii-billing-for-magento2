        ...
        if ($this->_helper->isBillwerkSubscriptionProduct($product)) {
            ...
            foreach ($quote->getAllItems() as $item) {
                if ($this->_helper->isBillwerkSubscriptionProduct($item->getProduct())) {
                    throw new LocalizedException(__('Your cart already contains a subscription product. Please complete your purchase or remove the existing subscription product before adding a new one.'));
                }
            }
        }
    }
